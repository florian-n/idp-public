...
import jax
from jax import random, lax, tree_map
...
from helpers import unwrapped_trajectories
from jax_md import simulate

# Hook certain JAX-MD functionality and replace it with own to enable
# calculation of unwrapped trajectories
unwrapped_trajectories.hook()

from jax_md import space, util, energy, units, rigid_body
...
def run_simulation(LJ_SIGMA_OO, N_STEPS, N_MOLECULES_PER_AXIS, N_SLICES, init_key):
    ...
    _, unwrapped_shift = space.periodic(BOX_SIZE, wrapped=False)
    ...
    def run_simulation(key, configuration, num_steps, timestep, tau):
        init_fn, step_fn = simulate.nvt_nose_hoover(
            energy_fn, shift, unwrapped_shift, timestep, kT, tau=tau
        )
        ...
        def step(state, t):
            ...
            real_unwrapped_positions, _ = rigid_body.union_to_points(
                state.unwrapped_position, shape
            )

            return (
                step_fn(state),
                (real_positions, real_unwrapped_positions),
            )
    ...
    atom_positions, unwrapped_atom_positions = simulation_result

    frame_r, dt_per_snapshot = get_slices(
        atom_positions, 3000, N_SLICES, TIMESTEP / unit["time"]
    )

    unwrapped_frame_r, _ = get_slices(
        unwrapped_atom_positions, 3000, N_SLICES, TIMESTEP / unit["time"]
    )

    return np.array(frame_r), np.array(unwrapped_frame_r), dt_per_snapshot
